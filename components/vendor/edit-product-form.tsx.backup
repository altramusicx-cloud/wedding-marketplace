...
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "../uicard"
import { Input } from "../uiinput"
import { Textarea } from "../uitextarea"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "../uiselect"
...
export function EditProductForm({ product, onSuccess }: EditProductFormProps) {
    ...
    const handleSelectChange = (name: keyof ProductFormData & string, value: string) => {
        setFormData(prev => ({ ...prev, [name]: value }))
        if (errors[name]) {
            setErrors(prev => ({ ...prev, [name]: '' }))
        }
